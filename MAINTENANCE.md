...
파일을 직접 열어 고칠 수도 있습니다. 이 문서의 각 작업에 순서가 적혀 있습니다. 다만 아래 「자동 검사가 실패할 수 있는 경우」를 꼭 읽어 주세요.
...
글을 공개해도 이메일은 **자동으로 나가지 않습니다.** Essay를 메일로 보내려면 직접 합니다.

1. 공개된 글의 주소를 확인합니다.
2. Buttondown(https://buttondown.com/sunwoochoi) 대시보드에서 새 이메일을 만들고 글 내용을 붙여 넣습니다.
3. 본문 끝에 글의 공개 주소를 넣습니다.
4. 본인에게 테스트 메일을 보내 휴대폰과 컴퓨터에서 확인합니다.
5. 확인한 뒤 직접 발송을 누릅니다.

Note는 기본적으로 보내지 않습니다. 구독자 이메일 주소와 Buttondown 비밀번호는 저장소나 채팅에 절대 붙여 넣지 마세요.
...
## 6. 자동 검사가 실패할 수 있는 경우

PR의 `verify` 검사는 사이트가 깨지지 않았는지 자동으로 확인합니다. 그런데 **지금은 글 목록과 글 제목을 검사에 그대로 적어 두었습니다.** 그래서 다음을 하면 검사가 실패합니다. 사이트가 고장 난 것이 아니라 검사가 옛 목록을 기준으로 하기 때문입니다.

- 새 글을 **공개**(`draft: false`)로 추가할 때. 글 개수와 폴더 목록을 확인하는 검사(`tests/unit/production-writing.test.ts`, `tests/e2e/writing.spec.ts`)가 실패합니다.
- 공개된 글의 제목을 바꾸거나 글을 지울 때.
- 홈, About, 구독 상자 같은 곳의 문구를 바꿀 때.

이럴 때는 Claude에게 "새 글을 추가했는데 검사가 실패해. 검사도 함께 고쳐 줘"라고 요청하세요. 글 내용만 고치는 오타 수정은 대개 실패하지 않습니다. 이 불편은 검사를 글 목록과 무관하게 바꾸면 없앨 수 있으니, 새 글을 자주 올리실 계획이면 Claude에게 그 개선을 요청하는 것을 권합니다.
