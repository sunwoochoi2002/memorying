...
`ko.md`와 `en.md`에는 각 언어의 제목과 본문을 따로 작성합니다. 원문 언어와 번역문을 모두 채워야 합니다. 사이트는 `originalLanguage`에 기록된 원문을 먼저 보여 주고, 독자는 같은 URL에서 한국어와 English 사이를 전환합니다.
...
초안 표시는 두 파일에서 모두 완성한 뒤에만 지우고, `meta.yaml`의 `draft: false`로 바꿉니다. 공개 글에는 `[Draft]` 제목을 남길 수 없습니다.
...
2. 원문을 해당하는 `ko.md` 또는 `en.md`에 복사해 Markdown 문법을 정리합니다.
3. 다른 언어 파일에 번역을 작성하고 제목과 본문을 모두 검토합니다.
...
사이트에 글을 공개해도 이메일은 자동 발송되지 않습니다. 메일은 항상 같은 최소 양식(원문 언어 제목, 본문 그대로, 구분선, `웹에서 읽기 · Read on the web`과 정식 URL, `Sunwoo Choi`)을 씁니다. `npm run newsletter -- <slug> [--lang ko|en]`가 `scripts/newsletter.mjs`로 이 양식을 만들어 제목을 출력하고 본문을 `.newsletter/<slug>.<lang>.md`(Git 무시)에 저장합니다. 발송은 Buttondown에서 직접 합니다.

1. 공개된 글에서 `npm run newsletter -- <slug>`를 실행합니다. 초안이나 없는 글은 거부됩니다.
2. Buttondown 대시보드에서 새 이메일을 만들고 제목과 본문을 그대로 붙여 넣습니다.
3. 데스크톱과 모바일에서 테스트 메일을 확인합니다.
4. 검토 후 Buttondown에서 직접 발송을 승인합니다.

Note도 같은 명령으로 메일을 만들 수 있으며, 보낼지는 직접 정합니다.
...
- [x] 최종 HTTPS 도메인을 정한 뒤 `SITE_URL`을 설정한다. (2026-09-19, `https://sunwoochoi.com`, `www`는 301로 대표 주소에 연결)
- [x] 실제 Buttondown 사용자명으로 `PUBLIC_BUTTONDOWN_USERNAME`을 설정한다.
- [x] 미리 보기와 실제 배포에서 sitemap, canonical URL, `_headers`, 구독 폼을 확인한다. (테스트 주소는 2026-09-18, 최종 도메인 `https://sunwoochoi.com`은 2026-09-19 확인 완료)
